...
require("paseo").setup {
  ...
  ui = {
    ...
    animate = {                   -- or `false` for instant
      ...
      reveal = true,              -- stagger rows in on a tab switch
      fps = 30,
    },
...
`ui.animate`, or `false` for instant. Three effects: a bar eased towards its
new value, a tool card tinted as it settles and fading out through the accent
ramp, and rows staggered in on a tab switch. The spinner is not one of them and
is never disabled — a turn that is running has to look different from a turn
that is wedged, and that is information rather than decoration.

None of the three changes a section's **height**, and that is a hard constraint
rather than a choice: volt records each section's start row when the layout is
...
A reveal therefore draws into rows that were already reserved.
