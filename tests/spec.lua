...
local function test_chat_follow()
  ...
  local ok, err = pcall(function()
    ...
    chat.open { root = here }
    truthy("follow: the chat opens on the directory asked for", vim.wait(1000, function()
      return chat.current() ~= nil and chat.current().agent_id == "agent-" .. vim.fs.basename(here)
    end))
    ...
    truthy("follow: onto the new workspace's agent", vim.wait(1000, function()
    ...
    vim.cmd.tabnew()
    chat.close()
    truthy("follow: closing the chat from another tab does not corrupt its own", (pcall(vim.cmd.tabclose)))
    ...
    eq("follow: `create` defaults back on for a chat you asked for", (function()
      local seen = false
      create.review = function()
        seen = true
      end
    ...
    truthy("follow: switching workspace moves the chat with it", vim.wait(1000, function()
